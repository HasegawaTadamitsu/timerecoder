...
public final class RecodeDao {
    ...
    public List<Recode> findByCategoryId(final int argCategoryId) {
        ...
        MyLog.getInstance()
                .readDatabase("categoryId[" + argCategoryId
                              + "]");
        String[] columns = {
                COLUMN_ID,
                COLUMN_DATE_TIME,
    ...
    public long insert(final Recode rec) {
        ...
        MyLog.getInstance()
                .writeDatabase("insertVal[" + rec
                               + "]");
        ContentValues values = new ContentValues();
        values.put(COLUMN_CATEGORY_ID,
                rec.getCategoryId());
        values.put(COLUMN_DATE_TIME,
                rec.getDateTime()
                        .toMilliSecond());
        values.put(COLUMN_EVENT_ID,
                rec.getEventIdToDBValue());
        values.put(COLUMN_MEMO,
                rec.getMemo());
